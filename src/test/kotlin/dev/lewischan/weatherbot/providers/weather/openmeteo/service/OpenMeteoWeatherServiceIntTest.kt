package dev.lewischan.weatherbot.providers.weather.openmeteo.service
...
class OpenMeteoWeatherServiceIntTest(
    ...
    test("get weather should return the current weather") {
        ...
        currentWeather.dailyWeather.date shouldBe LocalDate.parse("2024-10-31")
        currentWeather.dailyWeather.dailyTemperature.high.celsius shouldBe 32.2
    }

    test("get daily forecast should return tomorrow in the location timezone") {
        wireMockServer.stubFor(
            get(urlPathEqualTo("/v1/forecast"))
                .withQueryParam("forecast_days", equalTo("2"))
                .willReturn(
                    aResponse()
                        .withHeader("Content-Type", "application/json")
                        .withBody(javaClass.getResourceAsStream("/test-current-weather-response.json")!!.readAllBytes())
                        .withStatus(HttpStatus.OK_200)
                )
        )

        val forecast = openMeteoWeatherService.getDailyForecast(Location("", 1.3602148, 103.9871849), 1)

        forecast shouldNotBe null
        forecast!!.date shouldBe LocalDate.parse("2024-11-01")
        forecast.condition shouldBe Condition.LIGHT_SHOWERS
        forecast.precipitationProbability shouldBe 78
        forecast.dailyTemperature.low.celsius shouldBe 26.1
        forecast.dailyTemperature.high.celsius shouldBe 32.9
    }

    test("get daily forecast should reject a negative day offset") {
        shouldThrow<IllegalArgumentException> {
            openMeteoWeatherService.getDailyForecast(Location("", 1.3602148, 103.9871849), -1)
        }
    }

    test("get daily forecast should return null beyond the provider horizon") {
        openMeteoWeatherService.getDailyForecast(Location("", 1.3602148, 103.9871849), 16) shouldBe null
    }
