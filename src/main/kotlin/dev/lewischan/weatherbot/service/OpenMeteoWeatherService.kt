...
class OpenMeteoWeatherService(
    ...
    override fun getWeather(location: Location): CurrentWeather? {
        val response:OpenMeteoForecast? = openMeteoRestClient.get()
            .uri("/v1/forecast?latitude={latitude}&longitude={longitude}&current={current}&timeformat={timeFormat}", mapOf(
    ...
    fun currentWeatherMapper(openMeteoForecast: OpenMeteoForecast): CurrentWeather {
        return CurrentWeather(
            time = Instant.ofEpochSecond(openMeteoForecast.current.time),
